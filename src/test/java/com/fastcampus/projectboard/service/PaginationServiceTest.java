...
class PaginationServiceTest {
    ...
    @DisplayName("현재 페이저 번호와 총 페이지 수를 주면, 페이징 바 리스트를 만들어준다.")
    @MethodSource
    @ParameterizedTest(name = "[{index}] 현제 파이지: {0}, 총 페이지: {1} => {2}")
    void givenCurrentPageNumberAndTotalPages_whenCalculating_thenReturnPaginationBarNumbers(
        ...
        // When
        List<Integer> actual = sut.getPaginationBarNumbers(currentPageNumber,
            totalPages);
